...
#-------delete file-------
if os.path.exists('csv\\DataOutput.csv'):
    os.remove('csv\\DataOutput.csv')
...
for j, y in enumerate(myPicList):
    img = cv2.imread(path + "/" + y) #read user image
...
#-------create and write data in csv file-------
    with open('csv\\DataOutput.csv', 'a+') as f:
        for data in myData:
            data = str(data)
            data = data.replace('Mad', 'Md')
            f.write((data+', '))
